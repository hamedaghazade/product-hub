...
export const ProductForm: React.FC<Props> = ({ onProductCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: ProductCreateInput = {
      ...
      consumer_price: formData.consumer_price ? Number(formData.consumer_price) : null,
    };
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-600 mb-1.5">
            قیمت مصرف‌کننده (ریال) <span className="text-slate-400 font-normal">(اختیاری)</span>
          </label>
          <input
            type="number"
            min="0"
